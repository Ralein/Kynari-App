                ...
                <div className="space-y-4 animate-slide-up">
                    ...
                    {result.type === "face" && (
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/40 rounded-3xl p-5 flex items-center gap-4">
                            ...
                            <button onClick={() => { if (result.raw) faceResultRef.current = result.raw as unknown as AnalyzeImageResult; setCombinedMode(true); setActiveTab("audio"); setResult(null); setError(null); setSaved(false); }}
                    ...
                    <div className="bg-white/70 backdrop-blur-sm border border-white/80 shadow-[0_4px_24px_-4px_rgba(0,0,0,0.06)] rounded-3xl p-6 sm:p-8">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {!saved && selectedChild && (
                                <span className="text-xs text-slate-500">Saving for {children?.find(c => c.id === selectedChild)?.name}</span>
                            )}
                        </div>
                    </div>
                </div>
